...
class Test_ipv6(unittest.TestCase):
    ...
    def test_init_with_routing(self):
        self.setUp_with_routing()
        self.test_init()
    ...
    def test_parser_with_routing(self):
        self.setUp_with_routing()
        self.test_parser()
    ...
    def test_serialize_with_routing(self):
        self.setUp_with_routing()
        self.test_serialize()
        ...
        prev = None
        buf = self.ip.serialize(data, prev)
        #TODO MIN_LENはどう使われる？試験を動かせば分かるかも
    ...
    def test_len_with_routing(self):
        self.setUp_with_routing()
    ...
    def test_json_with_routing(self):
        self.setUp_with_routing()
        self.test_json()
...
class Test_routing_type3(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.routing = ipv6.routing(
            ...
            self.cmpe, self.pad, self.adrs)
        # create buf
        self.form = '!BBBBBB2x16s16s16s'
        self.buf = struct.pack(self.form, self.nxt, self.size,
                               self.type_, self.seg,
                               (self.cmpi << 4) | self.cmpe, self.pad,
...
#        form = '!BBBBBB2x4s4s4s'
    ...
    def test_serialize_with_compression(self):
        ...
        slice_e = slice(cmpe, 16)
        routing = ipv6.routing(
            nxt, size, type_, seg, cmpi, cmpe, pad, adrs)
        buf = routing.serialize()
        form = '!BBBBBB2x8s8s8s'
        res = struct.unpack_from(form, str(buf))
        eq_(nxt, res[0])
    ...
    def test_default_args(self):
        ...
        buf = hdr.serialize()
        LOG.info(repr(buf))
        res = struct.unpack_from(ipv6.routing._PACK_STR, str(buf))
        LOG.info(res)
