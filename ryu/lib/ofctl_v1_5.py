...
LOG = logging.getLogger('ryu.lib.ofctl_v1_5')
...
#    elif action_type == 'COPY_FIELD':
        ...
        result = parser.OFPActionMeter(meter_id)
    else:
        result = None
...
def action_to_str(act):
    ...
    if action_type == ofproto_v1_5.OFPAT_OUTPUT:
        buf = 'OUTPUT:' + str(act.port)
    elif action_type == ofproto_v1_5.OFPAT_COPY_TTL_OUT:
        buf = 'COPY_TTL_OUT'
    elif action_type == ofproto_v1_5.OFPAT_COPY_TTL_IN:
        buf = 'COPY_TTL_IN'
    elif action_type == ofproto_v1_5.OFPAT_SET_MPLS_TTL:
        buf = 'SET_MPLS_TTL:' + str(act.mpls_ttl)
    elif action_type == ofproto_v1_5.OFPAT_DEC_MPLS_TTL:
        buf = 'DEC_MPLS_TTL'
    elif action_type == ofproto_v1_5.OFPAT_PUSH_VLAN:
        buf = 'PUSH_VLAN:' + str(act.ethertype)
    elif action_type == ofproto_v1_5.OFPAT_POP_VLAN:
        buf = 'POP_VLAN'
    elif action_type == ofproto_v1_5.OFPAT_PUSH_MPLS:
        buf = 'PUSH_MPLS:' + str(act.ethertype)
    elif action_type == ofproto_v1_5.OFPAT_POP_MPLS:
        buf = 'POP_MPLS:' + str(act.ethertype)
    elif action_type == ofproto_v1_5.OFPAT_SET_QUEUE:
        buf = 'SET_QUEUE:' + str(act.queue_id)
    elif action_type == ofproto_v1_5.OFPAT_GROUP:
        buf = 'GROUP:' + str(act.group_id)
    elif action_type == ofproto_v1_5.OFPAT_SET_NW_TTL:
        buf = 'SET_NW_TTL:' + str(act.nw_ttl)
    elif action_type == ofproto_v1_5.OFPAT_DEC_NW_TTL:
        buf = 'DEC_NW_TTL'
    elif action_type == ofproto_v1_5.OFPAT_SET_FIELD:
        buf = 'SET_FIELD: {%s:%s}' % (act.key, act.value)
    elif action_type == ofproto_v1_5.OFPAT_PUSH_PBB:
        buf = 'PUSH_PBB:' + str(act.ethertype)
    elif action_type == ofproto_v1_5.OFPAT_POP_PBB:
        buf = 'POP_PBB'
#    TODO
#    elif action_type == ofproto_v1_5.OFPAT_COPY_FIELD:
#        buf = 'COPY_FIELD'
    elif action_type == ofproto_v1_5.OFPAT_METER:
        buf = 'METER' + str(act.meter_id)
...
def actions_to_str(instructions):
    ...
    for instruction in instructions:
        if isinstance(instruction,
                      ofproto_v1_5_parser.OFPInstructionActions):
            if instruction.type == ofproto_v1_5.OFPIT_APPLY_ACTIONS:
                for a in instruction.actions:
                    actions.append(action_to_str(a))
            elif instruction.type == ofproto_v1_5.OFPIT_WRITE_ACTIONS:
                write_actions = []
                for a in instruction.actions:
                    write_actions.append(action_to_str(a))
                if write_actions:
                    actions.append({'WRITE_ACTIONS': write_actions})
            elif instruction.type == ofproto_v1_5.OFPIT_CLEAR_ACTIONS:
                actions.append('CLEAR_ACTIONS')
            else:
                actions.append('UNKNOWN')
        elif isinstance(instruction,
                        ofproto_v1_5_parser.OFPInstructionGotoTable):
            buf = 'GOTO_TABLE:' + str(instruction.table_id)
            actions.append(buf)

        elif isinstance(instruction,
                        ofproto_v1_5_parser.OFPInstructionWriteMetadata):
...
def to_match_vid(value):
    ...
    if isinstance(value, int):
        ...
        return value | ofproto_v1_5.OFPVID_PRESENT
    else:
        if '/' in value:
            val = value.split('/')
            ...
            if value.isdigit():
                ...
                return int(value, 10) | ofproto_v1_5.OFPVID_PRESENT
            else:
                return int(value, 0)
...
def match_vid_to_str(value, mask):
    if mask is not None:
        ...
        if value & ofproto_v1_5.OFPVID_PRESENT:
            value = str(value & ~ofproto_v1_5.OFPVID_PRESENT)
        else:
            value = '0x%04x' % value
    return value
...
def stats_to_str(ofstats):
    ...
    for s in ofstats:
        key = s['OXSTlv']['field']
        if key in ['duration', 'idle_time']:
            value = {}
            value['duration_sec'] = s['OXSTlv']['value'][0]
            value['duration_nsec'] = s['OXSTlv']['value'][1]
        else:
...
def get_flow_stats(dp, waiters, flow={}):
    ...
    for msg in msgs:
        for stats in msg.body:
            stats_str = stats_to_str(stats.stats)
            match_str = match_to_str(stats.match)
            s = {'reason': stats.reason,
                 'stats': stats_str,
...
def get_port_stats(dp, waiters):
    prop_type = {dp.ofproto.OFPPSPT_ETHERNET: 'ETHERNET',
                 dp.ofproto.OFPPSPT_OPTICAL: 'OPTICAL',
                 dp.ofproto.OFPPSPT_EXPERIMENTER: 'EXPERIMENTER'}
    ...
    for msg in msgs:
        for stats in msg.body:
            ...
            for prop in stats.properties:
                p = {'type': prop_type.get(prop.type, 'UNKNOWN')}
                if prop.type == dp.ofproto.OFPPSPT_ETHERNET:
                    p['collisions'] = prop.collisions
                    p['rx_crc_err'] = prop.rx_crc_err
                    p['rx_frame_err'] = prop.rx_frame_err
                    p['rx_over_err'] = prop.rx_over_err
                elif prop.type == dp.ofproto.OFPPSPT_OPTICAL:
                    p['flags'] = prop.flags
                    p['tx_freq_lmda'] = prop.tx_freq_lmda
                    p['tx_offset'] = prop.tx_offset
                    p['tx_grid_span'] = prop.tx_grid_span
                    p['rx_freq_lmda'] = prop.rx_freq_lmda
                    p['rx_offset'] = prop.rx_offset
                    p['rx_grid_span'] = prop.rx_grid_span
                    p['tx_pwr'] = prop.tx_pwr
                    p['rx_pwr'] = prop.rx_pwr
                    p['bias_current'] = prop.bias_current
                    p['temperature'] = prop.temperature
                elif prop.type == dp.ofproto.OFPPSPT_EXPERIMENTER:
...
def mod_port_behavior(dp, port_config):
    ...
    type_convert = {'ETHERNET': ofp.OFPPDPT_ETHERNET,
                    'OPTICAL': ofp.OFPPDPT_OPTICAL,
                    'PIPELINE_INPUT': ofp.OFPPDPT_PIPELINE_INPUT,
                    'PIPELINE_OUTPUT': ofp.OFPPDPT_PIPELINE_OUTPUT,
                    'RECIRCULATE': ofp.OFPPDPT_RECIRCULATE,
                    'EXPERIMENTER': ofp.OFPPDPT_EXPERIMENTER}
    ...
    properties = []
    prop = port_config.get('properties')
    for p in prop:
        type_ = type_convert.get(p.get('type', 'ETHERNET'))
        if type_ == ofp.OFPPDPT_ETHERNET:
            m = parser.OFPPortModPropEthernet(type_=type_,
                                              advertise=p['advertise'])
        elif type_ == ofp.OFPPDPT_OPTICAL:
            m = parser.OFPPortModPropOptical(type_=type_,
                                             configure=p['configure'],
                                             freq_lmda=p['freq_lmda'],
                                             fl_offset=p['fl_offset'],
                                             grid_span=p['grid_span'],
                                             tx_pwr=p['tx_pwr'])
        elif type_ == ofp.OFPPDPT_EXPERIMENTER:
            m = parser.OFPPortModPropExperimenter(type_=type_,
                                                  experimenter=p[
                                                      'experimenter'],
                                                  exp_type=p['exp_type'],
                                                  data=p['data'])
        else:
            LOG.error('Unknown type: %s', p['type'])
        properties.append(m)

    port_mod = dp.ofproto_parser.OFPPortMod(
        dp, port_no, hw_addr, config, mask, properties)
