...
def action_to_str(act):
    ...
    if action_type == ofproto_v1_4.OFPAT_OUTPUT:
        buf = 'OUTPUT:' + str(act.port)
    elif action_type == ofproto_v1_4.OFPAT_COPY_TTL_OUT:
        buf = 'COPY_TTL_OUT'
    elif action_type == ofproto_v1_4.OFPAT_COPY_TTL_IN:
        buf = 'COPY_TTL_IN'
    elif action_type == ofproto_v1_4.OFPAT_SET_MPLS_TTL:
        buf = 'SET_MPLS_TTL:' + str(act.mpls_ttl)
    elif action_type == ofproto_v1_4.OFPAT_DEC_MPLS_TTL:
        buf = 'DEC_MPLS_TTL'
    elif action_type == ofproto_v1_4.OFPAT_PUSH_VLAN:
        buf = 'PUSH_VLAN:' + str(act.ethertype)
    elif action_type == ofproto_v1_4.OFPAT_POP_VLAN:
        buf = 'POP_VLAN'
    elif action_type == ofproto_v1_4.OFPAT_PUSH_MPLS:
        buf = 'PUSH_MPLS:' + str(act.ethertype)
    elif action_type == ofproto_v1_4.OFPAT_POP_MPLS:
        buf = 'POP_MPLS:' + str(act.ethertype)
    elif action_type == ofproto_v1_4.OFPAT_SET_QUEUE:
        buf = 'SET_QUEUE:' + str(act.queue_id)
    elif action_type == ofproto_v1_4.OFPAT_GROUP:
        buf = 'GROUP:' + str(act.group_id)
    elif action_type == ofproto_v1_4.OFPAT_SET_NW_TTL:
        buf = 'SET_NW_TTL:' + str(act.nw_ttl)
    elif action_type == ofproto_v1_4.OFPAT_DEC_NW_TTL:
        buf = 'DEC_NW_TTL'
    elif action_type == ofproto_v1_4.OFPAT_SET_FIELD:
        buf = 'SET_FIELD: {%s:%s}' % (act.key, act.value)
    elif action_type == ofproto_v1_4.OFPAT_PUSH_PBB:
...
def actions_to_str(instructions):
    ...
    for instruction in instructions:
        if isinstance(instruction,
                      ofproto_v1_4_parser.OFPInstructionActions):
            if instruction.type == ofproto_v1_4.OFPIT_APPLY_ACTIONS:
                for a in instruction.actions:
                    actions.append(action_to_str(a))
            elif instruction.type == ofproto_v1_4.OFPIT_WRITE_ACTIONS:
                write_actions = []
                for a in instruction.actions:
                    write_actions.append(action_to_str(a))
                if write_actions:
                    actions.append({'WRITE_ACTIONS': write_actions})
            elif instruction.type == ofproto_v1_4.OFPIT_CLEAR_ACTIONS:
                actions.append('CLEAR_ACTIONS')
            else:
                actions.append('UNKNOWN')
        elif isinstance(instruction,
                        ofproto_v1_4_parser.OFPInstructionGotoTable):
            buf = 'GOTO_TABLE:' + str(instruction.table_id)
            actions.append(buf)

        elif isinstance(instruction,
                        ofproto_v1_4_parser.OFPInstructionWriteMetadata):
            ...
            actions.append(buf)
                        ...
                        ofproto_v1_4_parser.OFPInstructionMeter):
...
def get_queue_stats(dp, waiters):
    ...
    prop_type = {dp.ofproto.OFPQSPT_EXPERIMENTER: 'EXPERIMENTER'}
    ...
    for msg in msgs:
        ...
        for stat in stats:
            ...
            for prop in stat.properties:
                ...
                if prop.type == dp.ofproto.OFPQSPT_EXPERIMENTER:
...
def get_queue_desc_stats(dp, waiters, port_no=None, queue_id=None):
    ...
    prop_type = {dp.ofproto.OFPQDPT_MIN_RATE: 'MIN_RATE',
                 dp.ofproto.OFPQDPT_MAX_RATE: 'MAX_RATE',
                 dp.ofproto.OFPQDPT_EXPERIMENTER: 'EXPERIMENTER',
                 }
    ...
    for config in msgs:
        ...
        for queue in config.body:
            ...
            for prop in queue.properties:
                p = {'type': prop_type.get(prop.type, 'UNKNOWN')}
                if prop.type == dp.ofproto.OFPQDPT_MIN_RATE or \
                   prop.type == dp.ofproto.OFPQDPT_MAX_RATE:
                    p['rate'] = prop.rate
                elif prop.type == dp.ofproto.OFPQDPT_EXPERIMENTER:
                    p['experimenter'] = prop.experimenter
                    p['exp_type'] = prop.exp_type
                    p['data'] = prop.data
                prop_list.append(p)
            q = {'port_no': queue.port_no,
...
def get_port_desc(dp, waiters):
    ...
    prop_type = {dp.ofproto.OFPPDPT_ETHERNET: 'ETHERNET',
                 dp.ofproto.OFPPDPT_OPTICAL: 'OPTICAL',
                 dp.ofproto.OFPPDPT_EXPERIMENTER: 'EXPERIMENTER'}
